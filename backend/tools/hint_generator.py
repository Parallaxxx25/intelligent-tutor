...
def _build_hint_prompt(
    ...
    prompt += (
        ...
        f"- If you invent a comparison/example query, use the student's actual schema "
        f"(BikeStores: customers, orders, order_items, products, staffs, stores) — "
        f"never a generic textbook schema like employees or departments\n"
...
def _level_3_concept(
    ...
    examples = {
        ...
        "aggregation_error": (
            ...
            "```sql\n"
            "-- ❌ 'last_name' is not in GROUP BY or aggregate\n"
            "SELECT store_id, last_name, COUNT(*)\n"
            "FROM staffs\n"
            "GROUP BY store_id;\n\n"
            "-- ✅ Option A: Add 'last_name' to GROUP BY\n"
            "SELECT store_id, last_name, COUNT(*)\n"
            "FROM staffs\n"
            "GROUP BY store_id, last_name;\n\n"
            "-- ✅ Option B: Use aggregate on 'last_name'\n"
